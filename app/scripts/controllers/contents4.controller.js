...
angular.module('churchApp')
    .controller('Contents4Ctrl', function ($scope, $routeParams, executeResults) {
        $scope.count = 0;
...
// church list load
        ...
        $scope.menuPage = '개별배정(지방회)';
        $scope.$emit('subMenuOn',$scope.menuPage);
        $scope.$watch('items', function (newV, oldV) {
            $scope.tryOrder = false;
        })
        $scope.chkMenu = function (data) {
            $scope.menuPage = data;
            $scope.subMenuOn = true;
            $scope.$emit('subMenuOn',$scope.menuPage);
        }
